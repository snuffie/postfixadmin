...
<h6><?php print $PALANG['pOverview_alias_alias_count'] . ": " . $limit['alias_count'] . " / " . $limit['aliases']; ?></h6>
<h6><?php print $PALANG['pOverview_alias_mailbox_count'] . ": " . $limit['mailbox_count'] . " / " . $limit['mailboxes']; ?></h6>
<form name="search" method="post" action="search.php">
   <div class="input-append">
      <input type="text" placeholder="<?php print $PALANG['pSearch']; ?>" name="search" size="10" />
...
{
   print "<table id=\"alias_domain_table\" class=\"table table-striped\">\n";
   print "   <tr>\n";
   ...
   if(sizeof ($tAliasDomains) > 0)
   {
      ...
      for ($i = 0; $i < sizeof ($tAliasDomains); $i++)
      {
         ...
         print "      <td style=\"padding-top: 12px;\">" . $tAliasDomains[$i]['modified'] . "</td>\n";
...
# TODO: change all edit-*.php scripts not to require the domain parameter (and extract it from the address). This avoids superflous problems when using search.
         ...
         print "      <td><a class=\"btn btn-small\" href=\"edit-active.php?alias_domain=true&domain=" . urlencode ($tAliasDomains[$i]['alias_domain']) . "&return=$file" . urlencode ( "?domain=" . $fDomain . "&limit=" . $current_limit) . "\">" . $active . "</a></td>\n";
         print "      <td><a class=\"btn btn-small btn-danger\" href=\"delete.php?table=alias_domain&delete=" . urlencode ($tAliasDomains[$i]['alias_domain']) . "&domain="
   ...
   if(is_array($tTargetDomain))
   {
      ...
      print "      <td style=\"padding-top: 12px;\">" . $tTargetDomain['modified'] . "</td>\n";
      $active = ($tTargetDomain['active'] == 1) ? $PALANG['YES'] : $PALANG['NO'];
      print "      <td><a class=\"btn btn-small\" href=\"edit-active.php?alias_domain=true&domain=" . urlencode ($fDomain) . "&return=$file" . urlencode ( "?domain=" . $fDomain . "&limit=" . $current_limit) . "\">" . $active . "</a></td>\n";
      print "      <td><a class=\"btn btn-small btn-danger\" href=\"delete.php?table=alias_domain&delete=" . urlencode ($fDomain) . "&domain=" . urlencode ($fDomain) . "\" onclick=\"return confirm ('" . $PALANG['confirm'] . $PALANG['pOverview_get_alias_domains'] . ": " . htmlentities ($fDomain) . "')\">" . $PALANG['del'] . "</a></td>\n";
      print "   </tr>\n";
...
# XXX: the above block misses one intention level
   if (!is_array($tTargetDomain))
   {
      ...
      print "<p><a class='btn' style='margin-top: 10px;' href=\"create-alias-domain.php?target_domain=$fDomain\">" . $PALANG['pMenu_create_alias_domain'] . "</a>\n";
   }
}

if (sizeof ($tAlias) > 0)
{
   print "<table id=\"alias_table\" class=\"table table-striped\">\n";
   print "   <tr>\n";
   ...
   for ($i = 0; $i < sizeof ($tAlias); $i++)
   {
      if ((is_array ($tAlias) and sizeof ($tAlias) > 0))
      {
         ...
         print "      <td style=\"padding-top: 12px;\">" . $tAlias[$i]['modified'] . "</td>\n";
...
# superadmin code
         ...
         $activetype = ($tAlias[$i]['active'] == 1) ? "btn-success" : "";
         print "      <td><a class=\"btn btn-small $activetype\" href=\"edit-active.php?alias=" . urlencode ($tAlias[$i]['address']) . "&domain=$fDomain&return=$file?domain=$fDomain" . urlencode ("&limit=" . $current_limit) . "\">" . $active . "</a></td>\n";
...
# domain admin code
         ...
         {
            $active = ($tAlias[$i]['active'] == 1) ? $PALANG['YES'] : $PALANG['NO'];
            print "      <td><a class=\"btn btn-small $activetype\" href=\"edit-active.php?alias=" . urlencode ($tAlias[$i]['address']) . "&domain=$fDomain" . "\">" . $active . "</a></td>\n";
            print "      <td><a class=\"btn btn-small\" href=\"edit-alias.php?address=" . urlencode ($tAlias[$i]['address']) . "\">" . $PALANG['edit'] . "</a></td>\n";
            print "      <td><a class=\"btn btn-small btn-danger\" href=\"delete.php?table=alias&delete=" . urlencode ($tAlias[$i]['address']) . "&domain=$fDomain" . "\"onclick=\"return confirm ('" . $PALANG['confirm'] . $PALANG['pOverview_get_aliases'] . ": ". $tAlias[$i]['address'] . "')\">" . $PALANG['del'] . "</a></td>\n";
         }
         else
...
if($tCanAddAlias) {
   print "<p><a class='btn' style='margin-top: 10px;' href=\"create-alias.php?domain=$fDomain\">" . $PALANG['pMenu_create_alias'] . "</a>\n";
}
...
if (sizeof ($tMailbox) > 0) {
   ...
   print "<table id=\"mailbox_table\" class=\"table table-striped\">\n";
   print "   <tr>\n";
   ...
   for ($i = 0; $i < sizeof ($tMailbox); $i++)
   {
      if ((is_array ($tMailbox) and sizeof ($tMailbox) > 0))
      {
         ...
         print "      <td style=\"padding-top: 12px;\">" . $tMailbox[$i]['modified'] . "</td>\n";
         $active = ($tMailbox[$i]['active'] == 1) ? $PALANG['YES'] : $PALANG['NO'];
         $activetype = ($tMailbox[$i]['active'] == 1) ? "btn-success" : "";
         print "      <td><a class=\"btn btn-small $activetype\" href=\"edit-active.php?username=" . urlencode ($tMailbox[$i]['username']) . "&domain=$fDomain" . "\">" . $active . "</a></td>\n";
         ...
         print "      <td><a class=\"btn btn-small\" href=\"edit-mailbox.php?username=" . urlencode ($tMailbox[$i]['username']) . "&domain=$fDomain" . "\">" . $PALANG['edit'] . "</a></td>\n";
         print "      <td><a class=\"btn btn-small btn-danger\" href=\"delete.php?table=mailbox" . "&delete=" . urlencode ($tMailbox[$i]['username']) . "&domain=$fDomain" . "\"onclick=\"return confirm ('" . $PALANG['confirm'] . $PALANG['pOverview_get_mailboxes'] . ": ". $tMailbox[$i]['username'] . "')\">" . $PALANG['del'] . "</a></td>\n";
         print "   </tr>\n";
      }
   }
   print "</table>\n";
...
if($tCanAddMailbox) {
   print "<p><a class='btn' style='margin-top: 10px;' href=\"create-mailbox.php?domain=$fDomain\">" . $PALANG['pMenu_create_mailbox'] . "</a>\n";
}
